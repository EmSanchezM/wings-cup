...
/**
 * Unit tests — 401 detection in useMatches composable (R-UX-03, scenario 2 + 4)
 *
 * T-64-01: 401 error → setExpired() called, error.value NOT set
 * T-64-02: non-401 error (500) → error.value IS set, setExpired NOT called
 */
...
describe('useMatches — 401 detection (R-UX-03)', () => {
  beforeEach(() => {
  ...
  it('T-64-01: 401 → setExpired() called, error.value NOT set', async () => {
  ...
  it('T-64-02: non-401 (500) → error.value IS set, setExpired NOT called', async () => {
    vi.stubGlobal('useSessionExpired', () => ({
      isExpired: _stateRef,
      setExpired: mockSetExpired,
      reset: mockReset,
